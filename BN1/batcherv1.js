...
const SCRIPTS = {
  hack: "hack-target.js",
  hWeaken: "weaken-target.js",
  grow: "grow-target.js",
  gWeaken: "weaken-target.js",
};
...
export async function main(ns) {
  ...
  while (true) {
    ...
    while (!isPrepped(ns, target)) {
      ...
      let w1threads = Math.max(Math.ceil(secDiff / 0.05), 1);

      //calculate how many grow threads we will need
      let multiplier =
        ns.getServerMaxMoney(target) / ns.getServerMoneyAvailable(target);
      let gthreads = Math.max(
        Math.ceil(ns.growthAnalyze(target, multiplier)),
        1
      );

      //calculate how many weaken threads after growing are needed
      let gSecDiff = gthreads * 0.004;
      let w2threads = Math.max(Math.ceil(gSecDiff / 0.05), 1);
      ...
      batchDetails.endTime["hWeaken"] =
        performance.now() +
        batchDetails.weakTime +
        batchDetails.jobBuffer * OFFSETS["hWeaken"] +
        1000;
      batchDetails.endTime["grow"] =
        performance.now() +
        batchDetails.weakTime +
        batchDetails.jobBuffer * OFFSETS["grow"] +
        1000;
      batchDetails.endTime["gWeaken"] =
        performance.now() +
        batchDetails.weakTime +
        batchDetails.jobBuffer * OFFSETS["gWeaken"] +
        1000;
    ...
    await scheduleBatch(ns, target, game);
...
class BatchDetails {
  ...
  update(greed = this.greed) {
    ...
    this.runTime.grow = this.weakTime * 0.8;
    ...
    const gThreads = Math.ceil(
      this.ns.growthAnalyze(
        this.target,
        this.maxMoney / (this.maxMoney - this.maxMoney * greed)
      )
    );
    this.threads.hack = hThreads;
    this.threads.grow = gThreads;
    this.threads.hWeaken = Math.max(Math.ceil((hThreads * 0.002) / 0.05), 1);
    this.threads.gWeaken = Math.max(Math.ceil((gThreads * 0.004) / 0.05), 1);
  }

  print() {
    this.ns.tprintf(
      "Target: %s hackRunTime: %d growRunTime: %d weakRunTime: %d hackEndTime: %d, hWeakenEndTime: %d, growEndTime: %d, gWeakenEndTime: %d",
      this.target,
      this.runTime.hack,
      this.runTime.grow,
      this.weakTime,
      this.endTime.hack,
      this.endTime.hWeaken,
      this.endTime.grow,
      this.endTime.gWeaken
    );
  }
}
    ...
    for (const server of servers) {
      if (ns.hasRootAccess(server)) {
        ...
        if (freeRam >= 1.7) {
          ...
          if (block.ram < this.#minSizeBlock) {
            this.#minSizeBlock = block.ram;
          }
          if (block.ram > this.#maxSizeBlock) {
            this.#maxSizeBlock = block.ram;
          }
...
async function scheduleBatch(ns, target, game) {
  let snapshot = new RamSnapshot(ns, getAllServers(ns));
  let batchDetails = new BatchDetails(ns, target, game.getNextBatchID());
  batchDetails.update();
  let batch = [];
  ...
  for (const action of ACTIONS) {
    ...
    batchDetails.endTime[action] =
      performance.now() +
      batchDetails.weakTime +
      batchDetails.jobBuffer * OFFSETS[action] +
      100;
